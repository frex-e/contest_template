...
signed main() {
	...
	REP(aaa,0,p) {
		...
		REP(i,0,n) {
			...
			// Find best line
			while(lines.size() >= 2 and (eval(lines[1], times[i]) >= eval(lines[0], times[i]))) {
					cout << "REMOVE " << eval(lines[1], times[i]) << " " << eval(lines[0], times[i]) << endl;
					lines.pop_front();
			}
			auto best_line = lines[0];
			cout << times[i] << " (" << best_line.first << ", " << best_line.second << ")";
			if (lines.size() >= 2) cout << " (" << lines[1].first << ", " << lines[1].second << ")";
			cout << "\n";
