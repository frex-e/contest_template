...
struct Node {
	...
	int len() { return r - l; }
	// More space efficient than map, cnan use alternatively.
	int& get(char c) {
		for (auto& [a, b] : next)
			if (a == c) return b;
		next.push_back({c, -1});
...
//     return usage.ru_maxrss; // Maximum resident set size (in kilobytes on
//     Linux, bytes on macOS)
// }

int n_words;
int par[10000005];
int uf_size[10000005];
...
vector<vector<int>> cnting_sort(4000005);
...
void dfs() {
	while (!stck.empty()) {
		...
		if (!hit_dollar) {
			if (index_to_word[cur_node.l] != index_to_word[cur_node.r - 1] ||
				index_to_word[cur_node.l] < 0) {
				hit_dollar = true;
		...
		// cout << " - " << cur << " - " << depth << " - " << depth_to_dollar;
		// cout << "\n";

		if (cur_node.next.empty()) {
			// cout << "JOINING " << index_to_word[s.size() - depth] << " " <<
			// get_indx(cur) << endl; cout << s[s.size() - depth] << endl; cout
			// << "B" << endl;
			join(index_to_word[s.size() - depth], get_indx(cur));
		} else {
			// cout << "D" << endl;
			cnting_sort[depth_to_dollar].push_back(cur);
			for (auto const& [a, b] : cur_node.next) {
				// cout << "E" << endl;
				// cout << b << " " << depth_to_dollar << " " << depth << " " <<
				// hit_dollar << endl;
...
signed main() {
	...
	for (int i = 0; i < n_words; i++) {
		...
		index_to_word.push_back(-i - 1);
		s.append(s_in);
		s.push_back('$');
	}
	distance_to_dollar.push_back(0);
	index_to_word.push_back(-n_words - 10);
	s.push_back('$');
	...
	for (int depth_to_dollar = cnting_sort.size() - 1; depth_to_dollar > 0;
		 depth_to_dollar--) {
		if (to_join == 0) break;
		for (int i = cnting_sort[depth_to_dollar].size() - 1; i >= 0; i--) {
			// cout << depth_to_dollar << endl;
			const auto& node = cnting_sort[depth_to_dollar][i];
			// cout << depth_to_dollar << " " << node << endl;
			to_join += 1;
			ans -= depth_to_dollar;

			for (auto const& [_, nxt] : t[node].next) {
				bool joined = join(get_indx(nxt), get_indx(node));
				// if (joined) cout << "JOINED " << nxt << " " << node << endl;
				to_join -= joined;
